...
function toggleMy() {
    elements = document.getElementsByClassName('myHideContainer');
    for (i = 0; i < elements.length; i++) {
        child = elements[i];
        child.classList.toggle('containerHidden');
    }
}

function myClicked(el, groupCheckName) {
    element = el;
    checkCheck = document.getElementById(groupCheckName);
    if (element.checked && !checkCheck.checked) {
        checkCheck.indeterminate = true;
    }
}

function myEnableChildren(el, divName) {
    element = el;
    div = document.getElementById(divName);
    if (div) {
        div.disabled = !element.checked;
        div.classList.toggle('myDisabledOption');
        for (i = 0; i < div.children.length; i++) {
            div.children[i].disabled = !element.checked;
        }
    }
} 
...
function menuActionMeldingListe(e, url, actionParameters) {
    ...
    document.getElementById('MELDING_ID').value = idInput.value;
    form = idInput.form;
    form.action = url;
    if (actionParameters) {
...
function taskIndikator() {
    $.ajax({
        url: '/Simulator/Dashboard/API/TaskCounter',
        type: 'get',
        success: function (data) {
        },
        complete: function (data) {
            setTimeout(taskIndikator, 120000);
        }
    });
}
